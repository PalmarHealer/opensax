...
// Lazily resolved so that vite build (which loads server modules with
// NODE_ENV=production but no runtime env) doesn't trip the prod fail-fast.
let _secret: string | null = null;
function resolveSecret(): string {
  if (_secret) return _secret;
  const env = process.env.ONLYOFFICE_JWT_SECRET?.trim();
  if (env && env.length >= 32) return (_secret = env);
...
export function ooSecret(): string { return resolveSecret(); }
...
export function ooMintToken(t: OoToken, ttlSec = 6 * 3600): string {
  return signJwt(t as unknown as Record<string, unknown>, resolveSecret(), ttlSec);
}
export function ooVerifyToken(token: string): OoToken | null {
  return verifyJwt<OoToken>(token, resolveSecret());
}
